...
const execute = async function (taskId) {
  ...
  let shares = secrets.share(id, pubKeys.length, minValidates);
  ...
  for (let index = 0; index < pubKeys.length; index++) {
    ...
    await this.actions.message.message(pubKeys[index], packet);
  }
