...
class Raft extends EventEmitter {
  constructor (address, options = {}) {
    ...
    raft.state = options.state || states.FOLLOWER;    // Our current state.
  ...
  _initialize (options) {
    ...
    raft.on('data', async (packet, write = () => {
      ...
      if (packet.term > raft.term) {
        raft.change({
          leader: states.LEADER === packet.state ? packet.address : packet.leader || raft.leader,
      ...
      if (packet.type === 'voted') {
        ...
        if (raft.quorum(raft.votes.granted)) {
          raft.change({leader: raft.address, state: states.LEADER});
     ...
     /*     if (hasIndex) raft.log.removeEntriesAfter(packet.last.index); //todo think about stage rules
      ...
      if (packet.type === 'task_vote') {
        this.actions.tasks.vote(packet.data.taskId, packet.data.share, packet.address, packet.term);
        return;
      }
      ...
      if (packet.type === 'task_executed') {
        this.actions.tasks.executed(packet.data.taskId, packet.data.payload, packet.address);
        return;
      }
