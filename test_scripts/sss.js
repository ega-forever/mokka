...
let pw = '123456789123456789'; //generated by node, which wants to commit the record

let nodesAmount = 3;

let privKeys = _.chain(new Array(nodesAmount)).fill(1).map(() => Wallet.generate().getPrivateKey().toString('hex')).value();

let start = Date.now();

let pwHex = secrets.str2hex(pw); // => hex string

let shares = secrets.share(pwHex, privKeys.length, Math.ceil(nodesAmount / 2) + 1);

console.log(shares)


let signedShares = shares.map((share, i) => { //each node sign it's share secret
  let {r, s, v, message} = web3.eth.accounts.sign(share, `0x${privKeys[i]}`);
