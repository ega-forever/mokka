...
module.exports = (ctx) => {
  ...
  it('check role change and heartbeat', async () => {
    ...
    await Promise.delay(1000 * (ctx.nodes.length + 1) + 2000);
    ...
    expect(itemsAfter).to.be.lt(ctx.nodes.length * 2);
  ...
  it('check role change and heartbeat (node boot delay)', async () => {
    ...
    for (let index = 0; index < ctx.ports.length; index++) {
      ...
      await Promise.delay(1000 * index);
    ...
    await Promise.delay(1000 * (ctx.nodes.length + 1) + 10000);
    ...
    expect(itemsAfter).to.be.lt(ctx.nodes.length);

  });

  it('check role change and heartbeat with pending logs', async () => {
